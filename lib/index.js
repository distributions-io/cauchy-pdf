...
function pdf( x, options ) {
	...
	if ( isNumber( x ) ) {
		return pdf1( x, opts.gamma, opts.x0 );
	}
	if ( isMatrixLike( x ) ) {
		if ( opts.copy !== false ) {
		...
		return pdf5( out, x, opts.gamma, opts.x0 );
	}
	if ( isTypedArrayLike( x ) ) {
		...
		return pdf6( out, x, opts.gamma, opts.x0 );
	}
	if ( isArrayLike( x ) ) {
		// Handle deepset first...
		if ( opts.path ) {
			opts.sep = opts.sep || '.';
			return pdf4( x, opts.gamma, opts.x0, opts.path, opts.sep );
		}
		...
		if ( opts.accessor ) {
			return pdf3( out, x, opts.gamma, opts.x0, opts.accessor );
		}
		return pdf2( out, x, opts.gamma, opts.x0 );
	}
	return NaN;
} // end FUNCTION pdf()
